...
module.exports = withDb(async function handler(req, res) {
  ...
  // ── Public endpoint: Myfxbook live stats (no auth required) ──
  const qAction = (req.query && req.query.action) || '';
  if (qAction === 'myfxbook_stats' && req.method === 'GET') {
    res.setHeader('Cache-Control', 'public, s-maxage=900, stale-while-revalidate=1800');
    const MFX_CACHE_KEY = 'myfxbook:stats_cache';
    const MFX_TTL = 4 * 60 * 60 * 1000; // 4 hours
    const MFX_ACCT = '11992323';
    const MFX_URL = 'https://www.myfxbook.com/members/Almudir/al-mudir/' + MFX_ACCT;

    // Check cache first
    const cached = await redis('GET', MFX_CACHE_KEY);
    if (cached) {
      let parsed;
      ...
      if (parsed && parsed.fetchedAt) {
        ...
        if (age < MFX_TTL) {
          return res.status(200).json({ ok: true, cached: true, cacheAge: Math.round(age / 60000) + ' min', stats: parsed });
        }
      }
    }

    // Fetch fresh from Myfxbook public page
    try {
      const ctrl = new AbortController();
      const tmr = setTimeout(() => ctrl.abort(), 12000);
      const mfxRes = await fetch(MFX_URL, {
        signal: ctrl.signal,
        headers: { 'User-Agent': 'Mozilla/5.0 (compatible; AlMudir/2.0)', 'Accept': 'text/html' }
      });
      clearTimeout(tmr);
      if (!mfxRes.ok) throw new Error('Myfxbook ' + mfxRes.status);
      const html = await mfxRes.text();

      const s = { gain:null, absGain:null, daily:null, monthly:null, drawdown:null, balance:null, equity:null, profit:null, pips:null, trades:null, wonPct:null, lostPct:null, profitFactor:null, deposits:null, withdrawals:null, lastUpdated:null };
      let m;
      m = html.match(/Gain[^<]*<[^>]*>[^<]*<[^>]*>\s*([\-\d,.]+)%/i);
      if (!m) m = html.match(/calItem[^>]*>Gain<\/[^>]*>[^<]*<[^>]*>\s*([\-\d,.]+)/i);
      if (m) s.gain = m[1].replace(/,/g, '');
      m = html.match(/Abs\.\s*Gain[^<]*<[^>]*>[^<]*<[^>]*>\s*([\-\d,.]+)%/i);
      if (m) s.absGain = m[1].replace(/,/g, '');
      m = html.match(/Daily[^<]*<[^>]*>[^<]*<[^>]*>\s*([\-\d,.]+)%/i);
      if (m) s.daily = m[1].replace(/,/g, '');
      m = html.match(/Monthly[^<]*<[^>]*>[^<]*<[^>]*>\s*([\-\d,.]+)%/i);
      if (m) s.monthly = m[1].replace(/,/g, '');
      m = html.match(/Drawdown[^<]*<[^>]*>[^<]*<[^>]*>\s*([\-\d,.]+)%/i);
      if (m) s.drawdown = m[1].replace(/,/g, '');
      m = html.match(/Balance[^<]*<[^>]*>[^<]*<[^>]*>\s*\$?([\-\d,.]+)/i);
      if (m) s.balance = m[1].replace(/,/g, '');
      m = html.match(/Equity[^<]*<[^>]*>[^<]*<[^>]*>\s*\$?([\-\d,.]+)/i);
      if (m) s.equity = m[1].replace(/,/g, '');
      m = html.match(/Profit[^<]*<[^>]*>[^<]*<[^>]*>\s*\$?([\-\d,.]+)/i);
      if (m) s.profit = m[1].replace(/,/g, '');
      m = html.match(/Pips[^<]*<[^>]*>[^<]*<[^>]*>\s*([\-\d,.]+)/i);
      if (m) s.pips = m[1].replace(/,/g, '');
      m = html.match(/Trades[^<]*<[^>]*>[^<]*<[^>]*>\s*([\d,]+)/i);
      if (m) s.trades = m[1].replace(/,/g, '');
      m = html.match(/Won[^<]*<[^>]*>\s*([\d,.]+)%/i);
      if (m) s.wonPct = m[1];
      m = html.match(/Lost[^<]*<[^>]*>\s*([\d,.]+)%/i);
      if (m) s.lostPct = m[1];
      m = html.match(/Profit\s*Factor[^<]*<[^>]*>[^<]*<[^>]*>\s*([\d,.]+)/i);
      if (m) s.profitFactor = m[1];
      m = html.match(/Deposits[^<]*<[^>]*>[^<]*<[^>]*>\s*\$?([\-\d,.]+)/i);
      if (m) s.deposits = m[1].replace(/,/g, '');
      m = html.match(/Withdrawals[^<]*<[^>]*>[^<]*<[^>]*>\s*\$?([\-\d,.]+)/i);
      if (m) s.withdrawals = m[1].replace(/,/g, '');
      m = html.match(/(?:Last\s+(?:Update|Updated)|Updated)[^<]*<[^>]*>\s*([^<]+)/i);
      if (m) s.lastUpdated = m[1].trim();

      s.fetchedAt = new Date().toISOString();
      s.accountId = MFX_ACCT;
      s.publicUrl = MFX_URL;

      await redis('SET', MFX_CACHE_KEY, JSON.stringify(s));
      return res.status(200).json({ ok: true, cached: false, stats: s });
    } catch (fetchErr) {
      // Serve stale cache on fetch failure
      if (cached) {
        let stale;
        try { stale = typeof cached === 'string' ? JSON.parse(cached) : cached; } catch (_) { stale = null; }
        if (stale) return res.status(200).json({ ok: true, cached: true, stale: true, stats: stale });
      }
      return res.status(502).json({ ok: false, error: 'Myfxbook unavailable', detail: String(fetchErr.message || '').slice(0, 200) });
    }
  }
